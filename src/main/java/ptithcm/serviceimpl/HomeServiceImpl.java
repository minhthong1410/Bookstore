...
public class HomeServiceImpl implements HomeService {
	...
	public int updateBook(BookEntity book) {
		return homeDao.updateBook(book);
	}
	
	@Override
	public List<BookEntity> SearchBook(String key){
		return homeDao.SearchBook(key);
	}
	
	@Override
	public int searchBookCount(String key) {
		return homeDao.searchBookCount(key);
	}
	
	@Override
	public int deleteBook(int bookID) {
		return homeDao.deleteBook(bookID);
	}
	
	@Override
	public int updateQuantyAfterCancel(int idbook, int quanty_order) {
		return homeDao.updateQuantyAfterCancel(idbook, quanty_order);
	}
	
	@Override
	public boolean checkDeleteBook(int bookID) {
		return homeDao.checkDeleteBook(bookID);
	}
	
	@Override
	public int updateBookQuanty(String hql) {
		return homeDao.updateBookQuanty(hql);
	}
}
