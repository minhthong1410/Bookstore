...
public class HomeDao {
	...
	public int updateQuantyAfterCancel(int idbook, int quanty_order) {
		Session session = factory.openSession();
		Transaction tx = session.beginTransaction();
        try {
			int current_quanty = getBookQuantybyID(idbook);
			int quanty = current_quanty + quanty_order;
			String hql = "UPDATE BookEntity B SET B.total_quantity = "+quanty+" WHERE B.id_book = "+idbook;
			Query query = session.createQuery(hql);
			query.executeUpdate();
			tx.commit();
		}catch (Exception e){
	...
	public int updateBookQuanty(ArrayList<CartDto> cart) {
        ...
        try {
        	...
        	System.out.println("UPDATE TON KHO THAT BAI..... "+ e);
        	tx.rollback();
        	return 0;
        } finally{
        	session.close();
	...
	public int updateBookQuanty(String hql) {
		Session session = factory.openSession();
		Transaction tx = session.beginTransaction();
        try {
    		Query query = session.createQuery(hql);
	...
	public List<BookEntity> SearchBook(String key){
		Session session = factory.getCurrentSession();
		String hql = "From BookEntity A WHERE A.book_name LIKE :key";
		Query query = session.createQuery(hql);
		query.setParameter("key", "%" + key + "%");
		@SuppressWarnings("unchecked")
		List<BookEntity> list = query.list();
	...
	public int searchBookCount(String key){
		Session session = factory.getCurrentSession();
		String hql = "SELECT count(A.id_book) From BookEntity A WHERE A.book_name LIKE :key";
		Query query = session.createQuery(hql);
		query.setParameter("key", "%" + key + "%");
		long soluong = (long) query.uniqueResult();
		return (int) soluong;
	...
	public boolean checkDeleteBook(int bookID) {
		Session session = factory.getCurrentSession();
		String hql = "From ItemEntity B WHERE B.id_book = "+bookID;
		Query query = session.createQuery(hql);
		@SuppressWarnings("unchecked")
		List<BookEntity> list = query.list();
		if(list.isEmpty() != true) {
		...
		hql = "From OrderDetailEntity B WHERE B.id_book = "+bookID;
		query = session.createQuery(hql);
		@SuppressWarnings("unchecked")
		List<BookEntity> list1 = query.list();
		if(list1.isEmpty() != true) {
			return false;
		}
		
		hql = "From ReviewEntity B WHERE B.id.id_book = "+bookID;
		query = session.createQuery(hql);
		@SuppressWarnings("unchecked")
		List<BookEntity> list2 = query.list();
	...
	public int deleteBook(int bookID){
		Session session = factory.getCurrentSession();
		String hql = "DELETE FROM BookEntity WHERE id_book = "+bookID;
		Query query = session.createQuery(hql);
